...
- shell 最小入口第一版已落地：新增 `runtime-core/task-cli.js`，当前已支持 `create / list / show / cancel / retry` 五条最小命令，并保持“先写事件、再更新快照”的账本纪律；同时补了 `demo-task-cli.js` 做真实闭环验证。当前验证结果为：`create` 可创建 `queued` 任务，`list` 可列出任务池，`show` 可展示状态与最近事件，`retry` 可将 `blocked` 任务转入 `retrying`，`cancel` 可将 `retrying` 任务终止为 `cancelled`。这说明 shell 入口最小闭环已接上 runtime，而不再只是规划文档。
- shell CLI 第二版与 demo 产物策略已补齐：项目 `.gitignore` 已新增 `runtime/task-ledger-*-demo/`，明确所有 demo ledger 目录默认不纳入版本管理；同时 `task-cli.js` 已新增 `request-review / approve / reject` 三条审批相关命令，接上 `ReviewCheckpoint` 与 review 决策闭环。对应样例 `demo-task-cli-review.js` 已验证：shell 可把任务从 `queued` 推入 `review_required`，随后通过 `approve` 带回 `ready`，或通过 `reject` 终止为 `cancelled`。
- 首次验收样例已真实跑通：新增 `runtime-core/demo-first-acceptance.js`，按主基线完成一条真实验收链 `queued -> planning -> ready -> running -> review_required -> ready -> running -> blocked -> retrying -> running -> done`，并补齐三类 done_check 后进入 `done`。最终验收结果为 `pass`，done_check 汇总为 `passed=3 / failed=0 / pending=0 / unresolved=0`；同时成功导出 `handoff / recap / archive` 三类文稿，且第三方复述校验已通过，说明当前最小 runtime 已具备“账本 + review + retry + 导出 + 验收”闭环能力。
